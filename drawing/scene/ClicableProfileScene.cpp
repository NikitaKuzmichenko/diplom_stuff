#include "ClicableProfileScene.h"
...
ClicableProfileScene::ClicableProfileScene(QObject *parent) : ClicableScene(parent){}

ClicableProfileScene::ClicableProfileScene(SimulationView *view,LayerManager *layerManager)
    : ClicableScene(view,layerManager){}

void ClicableProfileScene::displayBackgound(){
...
void ClicableProfileScene::updateBackgound(){
    bacgroundlayer->redraw();
}

void ClicableProfileScene::initVisiblaLayers(){
...
bool ClicableProfileScene::selectAnyItem(QPointF pos){
...
void ClicableProfileScene::initGrlidLayer(QImage img){
...
bool ClicableProfileScene::selectItem(QPointF pos){
    QGraphicsItem *i = itemAt(pos,QTransform());
...
bool ClicableProfileScene::releaseItem(QPointF pos){
...
bool ClicableProfileScene::moveItem(QPointF pos){
...
bool ClicableProfileScene::createItem(QPointF pos, NP::NodePointType type){
    view->createNewTurnPoint(pos,type);
    return true;
}

bool ClicableProfileScene::createItemOnSegment(QPointF pos, NP::NodePointType type){
    for(auto line : layerManager->getLayerByName(LayerName::line)->getItemGroupe()->childItems()){
        if(line->contains(pos)){
...
bool ClicableProfileScene::deleteItem(QPointF pos){
    QGraphicsItem *i = itemAt(pos,QTransform());
    for(auto layer : *layerManager->getLayers()){
        if(layer->getChangeable() && layer->contain(i)){
